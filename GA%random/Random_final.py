...
# ================================================================
# 7. Random Depot Selection
# ================================================================
def random_depot_search(G, num_depots=3, iterations=100, D_max=5000):
    """
    Randomly selects depot locations and evaluates routes.
    Keeps track of the best solution based on coverage and fitness.
    """
    nodes = list(G.nodes())
    best_score = -1
    best_solution = None
    ...
    for it in range(iterations):
        ...
        if coverage > best_coverage or (coverage == best_coverage and score > best_score):
            best_score = score
            best_solution = depots
            best_depot_routes = depot_routes
            best_depot_distances = depot_distances
            best_coverage = coverage

        print(f"Iter {it+1}: Fit={score:.3f}, Cov={coverage:.2f}%, Veh={total_vehicles}, Depots={depots}")

    total_vehicles = sum(len(routes) for routes in best_depot_routes.values())
    print("\n=== Best Overall Random Solution ===")
    ...
    plt.scatter(range(1, len(coverage_history) + 1), coverage_history, c='green', label='Coverage % per iteration')
    plt.plot(range(1, len(coverage_history) + 1), coverage_history, linestyle='--', alpha=0.7)
    plt.xlabel("Iteration")
    plt.ylabel("Coverage (%)")
    plt.title("Coverage Over Random Depot Iterations")
...
if __name__ == "__main__":
    ...
    D_max = 33333  # meters
    best_depots, best_depot_routes, total_vehicles, best_depot_distances = random_depot_search(
    G, num_depots=7, iterations=100, D_max=D_max)
